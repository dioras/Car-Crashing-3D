...
public class RCC_DetachablePart : MonoBehaviour{
	...
	RMD_Deformation rMD_Deformation;
	...
	void Start(){
		...
		rMD_Deformation = transform.root.GetComponent<RMD_Deformation>();
	...
	private void CheckJoint(){
		...
		if (isBreakable && strength <= detachPoint) {
			...
			if (joint) {
				...
				broken = true;
				Destroy (joint);
				Destroy(GetComponent<RMD_CollisionInvoker>());
				transform.SetParent (null);
				StartCoroutine(DisablePart(deactiveAfterSeconds));
				
				rMD_Deformation.PlayCrashSound();
			}
