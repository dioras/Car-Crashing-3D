...
public static class WMG_Anim
{
	public static void animFill(GameObject obj, float duration, Ease easeType, float animTo)
	{
		...
		{
			comp.fillAmount = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(false);
	}
	...
	{
		obj.transform.DOLocalRotate(animTo, duration, RotateMode.FastBeyond360).SetEase(easeType).SetUpdate(false).SetRelative(relative).OnComplete(onComp);
	}
	...
	{
		obj.transform.DOLocalRotate(animTo, duration, RotateMode.FastBeyond360).SetEase(easeType).SetUpdate(false).SetRelative(relative).OnUpdate(onUpd);
	}
	...
	public static void animPositionCallbackC(GameObject obj, float duration, Ease easeType, Vector3 animTo, TweenCallback onComp)
	{
		...
		}, animTo, duration).SetEase(easeType).SetUpdate(false).OnComplete(onComp);
	}
	...
	public static void animPositionCallbacks(GameObject obj, float duration, Ease easeType, Vector3 animTo, TweenCallback onUpd, TweenCallback onComp, string tid)
	{
		...
		{
			obj.transform.localPosition = x;
		}, animTo, duration).SetEase(easeType).SetUpdate(false).OnUpdate(onUpd).OnComplete(onComp).SetId(tid);
	}
	...
	public static void animScale(GameObject obj, float duration, Ease easeType, Vector3 animTo, float delay)
	{
		...
		}, animTo, duration).SetEase(easeType).SetUpdate(false).SetDelay(delay);
	}
