	...
	public class fsReflectedConverter : fsConverter
	{
		...
		public override fsResult TrySerialize(object instance, out fsData serialized, Type storageType)
		{
			...
			for (int i = 0; i < fsMetaType.Properties.Length; i++)
			{
				...
				{
					fsData value;
					fsResult result = this.Serializer.TrySerialize(fsMetaProperty.StorageType, fsMetaProperty.Read(instance), out value);
			...
			for (int i = 0; i < fsMetaType.Properties.Length; i++)
			{
				...
				if (fsMetaProperty.CanWrite)
				{
					...
					if (data.AsDictionary.TryGetValue(fsMetaProperty.JsonName, out data2))
					{
						...
						fsResult result = this.Serializer.TryDeserialize(data2, fsMetaProperty.StorageType, ref value);
