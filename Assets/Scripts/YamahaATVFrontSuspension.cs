...
public class YamahaATVFrontSuspension : Suspension
{
	...
	private void FixedUpdate()
	{
		...
		Vector3 localPosition = this.FLWheel.Dummy.localPosition;
		localPosition.z = this.FLWheel.Dummy.parent.InverseTransformPoint(this.wheelColliders[0].GetVisualWheelPosition()).z - 0.09f;
		localPosition.y = this.FLWheel.Knuckle.localPosition.y;
		...
		this.FRWheel.TieRod.LookAt(this.SteeringRack, base.transform.forward);
		Vector3 visualWheelPosition = this.wheelColliders[1].GetVisualWheelPosition();
		Vector3 position = this.FRWheel.LowerArm.parent.InverseTransformPoint(visualWheelPosition);
		position.x = this.FRWheel.LowerArm.localPosition.x;
		position.z -= 0.09f;
		float num2 = Vector3.Distance(this.FRWheel.LowerArm.position, this.FRWheel.KnucklePosition.position);
		float num3 = position.z - this.FRWheel.LowerArm.localPosition.z;
		position.y = this.FRWheel.LowerArm.localPosition.y + Mathf.Sqrt(num2 * num2 - num3 * num3);
		position.y /= base.transform.lossyScale.x;
		Vector3 vector = this.FRWheel.LowerArm.parent.TransformPoint(position);
		this.FRWheel.Dummy.position = vector;
		Quaternion rotation = Quaternion.LookRotation(vector - this.FRWheel.LowerArm.position, base.transform.forward);
		this.FRWheel.LowerArm.rotation = rotation;
		localPosition = this.FRWheel.Dummy.localPosition;
		localPosition.z = this.FRWheel.Dummy.parent.InverseTransformPoint(this.wheelColliders[1].GetVisualWheelPosition()).z - 0.09f;
		localPosition.y = this.FRWheel.Knuckle.localPosition.y;
		if (localPosition.y < 0f)
		{
			localPosition.y = -localPosition.y;
		}
